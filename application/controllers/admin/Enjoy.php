...
class Enjoy extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->model($this->ldir.'/enjoy_model');
    ...
    public function list($page = 1)
    {
        ...
        //이전 페이지 마지막 데이터
        if($page > 1) {
            $temp = $this->enjoy_model->enjoyListData($page - 1);
            $pre_one = [end($temp)];
        } else {
            $pre_one = [];
        }
        $list_data = $this->enjoy_model->enjoyListData($page);
        $list_total_num = $this->enjoy_model->enjoyTotalNum();
        //다음 페이지 첫번째 데이터
        $next_one = $this->enjoy_model->enjoyListData($page + 1);
...
//            $this->json(array('return'=>false, 'err_msg'=>'데이터 제한수를 넘었습니다.'));
    ...
    public function addData()
    {
        ...
        $json = $this->enjoy_model->enjoyAddData($insert_data);
        $this->json($json);
    }
    ...
    {
        $name = "Enjoy 수정";
        $target_data = $this->enjoy_model->enjoyData($idx);
        $data = [
            'name'=>$name,
            'data'=>$target_data,
            'file_func' => $this->file_func,
        ...
        $this->load->view($this->ldir . '/admin_header');
        $this->load->view($this->ldir. '/enjoy_add',$data);
        $this->load->view($this->ldir . '/admin_footer');
    }
    ...
    public function editData()
    {
        ...
        $json = $this->enjoy_model->enjoyEditData($update_data, $idx);
    ...
    {
        $json = $this->enjoy_model->enjoydel($idx);
        $this->json($json);
    }
    public function count()
    {
        $json = $this->enjoy_model->enjoyCount();
        $this->json($json);
    }
    ...
    public function order()
    {
        ...
        $json = $this->enjoy_model->enjoyOrder($idxs, $idx);
        $this->json($json);
    }
