                    ...
                    <div id="myTabContent" class="tab-content">
                        <?foreach ($category as $sym=>$val):?>
                            ...
                            <div role="tabpanel" class="tab-pane fade <?if($sym == $curr_cate) echo 'active in'; ?>" id="<?='category_'.$sym?>" aria-labelledby="<?='category_'.$sym?>-tab">
                                ...
                                <div class="x_title">
                                    <?if($insert) {?>
                                        <button type="button" class="btn btn-info" id="charge_addBtn">등록하기</button>
                                    <?}?>
                                    <button type="button" id="idxs_<?=$sym?>go" class="btn btn-primary" data-url="/admin/charge/order" data-count="/admin/charge/count" data-page="<?=$page?>">순서 저장하기</button></a>
                                    <div class="clearfix"></div>
                                </div>
                                ...
                                <table id="tbl_<?=$sym?>" class="table table-striped table-bordered">
                                    ...
                                    <?if(empty($list[$sym])):?>
                                        <tr class="odd"><td valign="top" colspan="8" class="dataTables_empty">데이터가 없습니다.</td></tr>
                                    <?else:?>
...
<script>
    $('body').on('click', '.adminedit', function() {
        ...
        location.href = '/admin/charge/edit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        ...
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/admin/charge/datadel/'+idx);
            if(result.return == true) {
                alert("삭제되었습니다");
                location.href = '/admin/charge/list?curr_cate='+category;
            }
        }
    });
    ...
    function js_page(page,category) {
        ...
        var option = {
            url : '/admin/charge/listPage/'+page + '/'+category,
            async:false
        };
        option.type = "post";
